...
def match_readings(hardware: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not any(_finite(v) for v in values) and hardware.get('model') != 'truepill-snapshot':
...
def match_sentence(match: dict[str, Any], *, include_distance: bool = True) -> str:
    ...
    text = f"Closest match: {match['closest_match']}. This is a synthetic reference comparison, not confirmed identity."
    if include_distance:
        text += f" Distance {match['distance']:.3f} absorbance."
