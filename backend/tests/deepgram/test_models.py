...
from backend.deepgram.models import ConcernReportCreate, PurchaseLocation


def test_empty_payload_leaves_provenance_blank() -> None:
    body = ConcernReportCreate.model_validate({})
    assert body.purchased_on is None
    assert body.purchase_location is None
    assert body.seller is None


def test_unwraps_deepgram_arguments() -> None:
    body = ConcernReportCreate.model_validate(
        {
            "arguments": {
...
def test_location_string_becomes_a_label() -> None:
    body = ConcernReportCreate.model_validate({"purchase_location": "the CVS in Cambridge"})
    assert body.purchase_location is not None
...
def test_month_only_date_becomes_the_first() -> None:
    body = ConcernReportCreate.model_validate({"purchased_on": "2026-03"})
    assert body.purchased_on == date(2026, 3, 1)


def test_unknown_date_is_omitted() -> None:
    body = ConcernReportCreate.model_validate({"purchased_on": "not sure"})
    assert body.purchased_on is None
