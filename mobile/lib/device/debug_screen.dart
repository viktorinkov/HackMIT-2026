...
class _DebugScreenState extends State<DebugScreen> {
  ...
  Widget _commands(Session session) => Wrap(
    spacing: 8,
    runSpacing: 8,
    children: [
  ...
  Widget _faults(List<Fault> faults) => _faultList(
    'faults',
    faults.where((f) => f.severity != Severity.info).toList(),
  );
  ...
  Widget _diag(Diag? d) {
    if (d == null) {
      return const Text('No structured diagnostics reported by this firmware.');
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        _row(
          'heap',
          '${d.heapBytes} (min ${d.heapMinBytes})  loopMax ${d.loopMaxUs} us',
        ),
        ...
        _row(
          'motor',
          'shift trans ${d.motorTransShiftMv}, scat ${d.motorScatShiftMv}',
        ),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (session.history.unknown.isNotEmpty) ...[
          ...
          for (final line in session.history.unknown.reversed.take(5))
            Text(line.text),
        ],
      ],
    );
